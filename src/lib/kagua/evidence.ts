...
function shortTopicQuery(r:AnalysisRequest):string{
  // Title, abstract and a full manuscript paste are each independently
  // optional now (a researcher can search on any one alone) — keywords
  // first, then whichever of title/abstract/manuscript is actually
  // present, so this never degenerates to an empty search string.
  ...
  const fromManuscript=txt(r.manuscript||"").split(/\s+/).filter(Boolean).slice(0,6).join(" ");
  return txt(fromKeywords||fromTitle||fromAbstract||fromManuscript).slice(0,120);
}
