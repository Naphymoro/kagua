...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return <html lang="en"><body><nav className="appTabs" aria-label="Kagua sections"><div className="appTabsLinks"><Link href="/">Journal Hunter</Link><Link href="/registry">DHET List</Link><Link href="/manual">Manual</Link></div><div className="authNav">{session?.user?.email ? <><span className="chip">{session.user.email}</span><form action={async()=>{"use server";await signOut({redirectTo:"/sign-in"})}}><button className="textButton" type="submit">Sign out</button></form></> : <Link className="textButton" href="/sign-in">Sign in</Link>}</div></nav>{children}<footer className="siteFooter"><p>Developed and maintained by <strong>Prof. Cecil Ouma</strong>, Academic and Research Manager, AIMS Research and Innovation Centre</p></footer></body></html>;
}
