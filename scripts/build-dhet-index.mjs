...
let workbookBuffer;
const fetchTimeoutMs = Number.isFinite(FETCH_TIMEOUT_MS) ? FETCH_TIMEOUT_MS : 45000;
let timeout;
try {
  const response = await Promise.race([
    fetch(SOURCE_URL, { headers: { 'User-Agent': 'Kagua/1.0 DHET index builder' } }),
    new Promise((_, reject) => {
      timeout = setTimeout(() => reject(new Error(`DHET workbook fetch timed out after ${fetchTimeoutMs}ms`)), fetchTimeoutMs);
    }),
  ]);
